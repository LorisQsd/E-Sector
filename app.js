  ...
  if (searchedString.length >= 3) {
    try {
      ...
      const result = data.filter((city) =>
        city.streetName.includes(searchedString)
      );

      if (result.length) {
        resultMsg.textContent =
          "Nombre de" + result.length > 1
            ? "résultats:"
            : "résultat:" + result.length;
        resultMsg.textContent = `Nombre de ${
          result.length > 1 ? "résultats" : "résultat"
        }: ${result.length}`;
    ...
    tbody.insertAdjacentHTML(
      ...
      `
        <tr>
            ${
              isInterval
                ? `<td class="td--interval">${result.streetInfo.interval}</td>`
                : ""
            }
            ...
            <td><a title="Cliquez pour voir la géolocalisation" class="td--gmap" href="https://www.google.fr/maps/place/${
              result.streetName
            }+${city}" target="_blank"><i class="bi bi-geo-alt-fill"></i> Visualiser</a></td>
        </tr>
        `
    );
  });
